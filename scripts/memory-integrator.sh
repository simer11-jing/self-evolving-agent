...
if [ ! -f "$MEMORY_DIR/scripts/memory-consolidate.js" ]; then
...
if [ "$MEMORY_LINES" -gt "$MAX_LINES" ] || [ "$MEMORY_SIZE" -gt "$((MAX_SIZE_KB * 1024))" ]; then
    ...
    node "$MEMORY_DIR/scripts/memory-consolidate.js"
    echo "✅ 规整完成"
else
    echo "✅ MEMORY.md 状态正常"
fi
...
# 检查日志归档
if [ ! -f "$MEMORY_DIR/scripts/memory-archive.js" ]; then
...
if [ "$LOG_COUNT" -gt 20 ]; then
    ...
    node "$MEMORY_DIR/scripts/memory-archive.js"
    echo "✅ 归档完成"
else
    echo "✅ 日志文件数量正常"
fi
